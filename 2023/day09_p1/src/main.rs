...
fn find_sequence_continuation(numbers: &Vec<i32>) -> i32 {
    ...
    let mut next_value_to_apply = i32::MAX;
    while continuation.len() > 0 {
        let last = continuation.pop().unwrap();
        if next_value_to_apply != i32::MAX {
            next_value_to_apply = next_value_to_apply + last[last.len() - 1];
        } else {
            next_value_to_apply = last[last.len() - 1];
        }
    }
